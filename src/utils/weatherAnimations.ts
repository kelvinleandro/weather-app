...
const weatherConditionMap: { [key: number]: { day?: DayAnimations; night?: NightAnimations ; common?: CommonAnimations } } = {
...
export function getWeatherAnimation(conditionCode: number, isDay: number) {
  ...
  if (mappedCondition) {
    if (mappedCondition.common) {
      return animations.common[mappedCondition.common];
    }
    
    const timeOfDayAnimation = mappedCondition[timeOfDay];
    if (timeOfDayAnimation) {
      return animations[timeOfDay][timeOfDayAnimation];
    }
  }
  // Default to a placeholder animation if condition not found
  return timeOfDay === 'day' ? animations.day.sunny : animations.night.clear;
}